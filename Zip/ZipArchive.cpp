	...
	if (!info.m_szFilePath.IsEmpty()) // it may be empty after removing sep.
	{
		...
		if (info.m_szFileNameInZip.IsEmpty())
		{
			...
			if (info.m_bFullPath)
			{
				...
				//CZipString f= RemoveRootPath(zpc);
...
CZipString CZipArchive::RemoveRootPath(CZipPathComponent &zpc)
{
	if (m_szRootPath.IsEmpty())
		return zpc.GetFileName();
	...
	CZipString szBeginning(m_szRootPath);
	...
	if (iRootPathLength && szPath.GetLength() >= iRootPathLength &&
		(szPath.Left(iRootPathLength).*m_pZipCompare)(szBeginning) == 0)
	{		
