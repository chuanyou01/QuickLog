#ifndef	LIBCHEN_HASHTABLE_H_
#define LIBCHEN_HASHTABLE_H_
...
namespace chen{
	...
	class hashelement{
		friend static void  printelement(const hashelement<chen::word,int> &element);
	public:
		hashelement()
		{
		}
	...
	private:
		...
		static void  printelement(const hashelement<chen::word,int> &element)
		{
	...
	public:
		...
		hashelement<KEY_,VALUE_> &getelement(void)
		{
			...
			{
				chen::treenode<hashelement<KEY_,VALUE_> > *pnode=NULL;
	...
	/*	void pushnext(const KEY_ &key,const VALUE_ &value)
		...
		bool find(const KEY_ &key,hashelement<KEY_,VALUE_>**pelement)
		{
			...
			chen::treenode<hashelement<KEY_,VALUE_> > *pnode=m_otherelements.findnode(element);
			if(pnode==NULL)
				return false;
	...
	private:
		...
		chen::sorttree<hashelement<KEY_,VALUE_> > m_otherelements;
		bool m_eraseflag;
	...
	public:
		...
		VALUE_ * find(const KEY_ &key)
		{
			...
			S_UINT nHash=HashKey(key)%m_tablesize;
		...
		class iterator : public chen::hiterator<HELEMENT >
		{
	...
	/*	bool findfirst(iterator &it)
		...
		bool findnext(iterator &it)
		{
			...
			for(int i=it.m_pos+1;i<(int)ncount;i++)
			{
				if(m_nodes[i]!=NULL)
				{
					it.m_pos=i;
	...
	private:
		...
		inline unsigned int HashKey(const char * key)
		{
		...
		//inline unsigned int HashKey(const unsigned int key)
		//{
		//	unsigned int nkey=key;
		//	unsigned int nret=((unsigned int)(void*)(unsigned int)nkey) >> 4;
		//	return nret;
		//}

		inline unsigned long HashKey2(const char * key)
		{
			unsigned long h = 0; 
			for ( ; *key; ++key)
				h = 5*h + *key;
		...
		inline unsigned int HashKey(const chen::word & key)
		{
			return HashKey((char *) key);
		}
		inline unsigned int HashKey(unsigned int key)
		{
			unsigned int nret= (key* 2654435761UL);
			return nret;
		}
		inline unsigned int HashKey(__int64 key)
		{
			unsigned int low=(unsigned int)key;
			unsigned int high=(unsigned int)(key>>32);

			return ((low | high)*2654435761UL);
		}
